...
app.get('/upload_already', async (req, res) => {
    ...
    let path = `${uploadDir}/${HASH}`,
        pathName=pathssss.resolve(uploadDir, `${HASH}${suffix}`)
        fileList = [];
    try {
        fileList = fs.readdirSync(path);
        ...
        res.send({
            code: 0,
            // fileList
            shouldUpload:fs.existsSync(pathName)?false:true,
            fileList: fs.existsSync(pathName)?null:fileList
        });
    } catch (err) {
        res.send({
            code: 0,
